...
class Basket extends ComponentBase
{
    ...
    public function onAddToBasket() {
        ...
        // QUANTITY
        // ---------------------------------------------------------------------
        // get or set default qty  to 1
        $qtyOperation = input("default_quantity_operation", null);
        if ($qtyOperation == null) { 
            $qtyOperation = 1; 
        }
        else {
            // remove from $allInputs because QTY should not be used for a full key product
            unset($allInputs["default_quantity_operation"]);
        }
        
        // check minimal order
        if ($product->minimum_quantity > $qtyOperation) {
        ...
        $fullKeyProduct = md5(json_encode($allInputs));
    ...
    public function onAddCouponCode() {
        ...
        // now remove invalid coupon from session - message will be display only one time
        if (array_get($basket, "coupon_model", null) == null) {
            $basket["coupon_code"] = "";
            $basket = $this->setSessionBasket($basket);
        }
