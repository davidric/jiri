...
//
    ...
    /**
     * Duplicate product with all relations
     * 
     * @return boolean - all ok / error
     */
    public function duplicate() {
        

        $duplicatedProduct = $this->replicate();

        // try to find new available copy number
        $finalIndex = null;
        for ($index = 1; $index < 1000; $index++) {
            $wSlug = $duplicatedProduct->slug . '-duplicate-'.$index;
            if (self::where("slug", $wSlug)->count() == 0) {
                $finalIndex = $index;
                break;
            }
        }
        ...
        if ($finalIndex != null) {
            $duplicatedProduct->slug = $duplicatedProduct->slug . '-duplicate-'.$finalIndex;
            $duplicatedProduct->title = $duplicatedProduct->title . '-duplicate-'.$finalIndex;
            $duplicatedProduct->save();
            
            // add relations
            $duplicatedProduct->categories()->sync($this->categories);
            $duplicatedProduct->accessories()->sync($this->accessories);
            $duplicatedProduct->featured()->sync($this->featured);
            $duplicatedProduct->properties()->sync($this->properties);
            $duplicatedProduct->propertyOptions()->sync($this->propertyOptions);
