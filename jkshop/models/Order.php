...
//        }
     ...
     * @param type $id
     */
    public static function findOrderBySlugForPaymentGateway($slug) {
        
        // get order if id and security token is OK
        $idArr = explode("-", $slug);
        $security_token = array_get($idArr, 0, null);
        $orderId = array_get($idArr, 1, null);
        $order = self::where("security_token", $security_token)->where("id",$orderId)->first();
        if ($order == null) {
            return false;
        }
